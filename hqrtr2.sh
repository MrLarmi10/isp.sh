#!/bin/bash
# hq-rtr_setup.sh — исправленная версия
...
# Проброс порта 8080 на веб-приложение HQ-SRV
if ! nft list chain inet nat prerouting | grep -q "dport 8080 dnat ip to 192.168.1.2:80"; then
    nft add rule inet nat prerouting tcp dport 8080 dnat ip to 192.168.1.2:80
fi

# Проброс порта 3226 на SSH (HQ-SRV, порт 2026)
if ! nft list chain inet nat prerouting | grep -q "dport 3226 dnat ip to 192.168.1.2:2026"; then
    nft add rule inet nat prerouting tcp dport 3226 dnat ip to 192.168.1.2:2026
fi
