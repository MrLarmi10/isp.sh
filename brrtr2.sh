...
# Проброс порта 8080 на testapp (BR-SRV, порт 80)
if ! nft list chain inet nat prerouting | grep -q "dport 8080 dnat to 192.168.4.2:80"; then
    nft add rule inet nat prerouting tcp dport 8080 dnat to 192.168.4.2:80
fi

# Проброс порта 3226 на SSH (BR-SRV, порт 2026)
if ! nft list chain inet nat prerouting | grep -q "dport 3226 dnat to 192.168.4.2:2026"; then
    nft add rule inet nat prerouting tcp dport 3226 dnat to 192.168.4.2:2026
fi
