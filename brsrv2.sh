#!/bin/bash
# br-srv_setup.sh - без настройки IP
...
# Настройка домена (без явного указания интерфейсов)
samba-tool domain provision --use-rfc2307 --realm=AU-TEAM.IRPO --domain=AU-TEAM --adminpass='P@ssword' --server-role=dc --dns-backend=SAMBA_INTERNAL --option="bind interfaces only=no"
...
EOF
# Проверка связи (игнорируем ошибки DNS, но pong должен быть)
