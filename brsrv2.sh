...
# ============================================
# 4. Настройка Ansible (установка если отсутствует)
# ============================================
...
if ! command_exists sshpass; then
    ...
    if ! dnf install -y sshpass; then
        echo "sshpass не найден в репозиториях, собираем из исходников..."
        cd /tmp
        curl -LO https://sourceforge.net/projects/sshpass/files/sshpass/1.09/sshpass-1.09.tar.gz
        tar xzf sshpass-1.09.tar.gz
        cd sshpass-1.09
        ./configure
        make && make install
        cd /tmp && rm -rf sshpass-1.09*
    fi
fi
...
if ! command_exists ansible; then
    ...
    dnf install -y epel-release || true
    dnf install -y ansible-core || dnf install -y ansible || {
        echo "Не удалось установить Ansible через DNF, пробуем через pip3..."
        dnf install -y python3-pip
        pip3 install ansible
        # Добавляем путь для текущей сессии
        export PATH=$PATH:~/.local/bin
    }
fi

# Проверка
if command_exists ansible && command_exists sshpass; then
    echo "Ansible и sshpass успешно установлены."
else
    echo "Ошибка: не удалось установить ansible или sshpass. Выход."
    exit 1
fi
...
# Тест пинга (с игнорированием ошибок доступности)
