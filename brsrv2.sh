...
# Удаляем podman-docker, если он мешает
if rpm -q podman-docker &>/dev/null; then
    echo "Удаляем конфликтующий пакет podman-docker..."
...
# Если Docker не установился, пробуем через официальный скрипт
...
# Если docker-compose всё ещё отсутствует, ставим из GitHub
