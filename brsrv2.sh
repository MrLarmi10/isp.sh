...
[servers]
HQ-SRV ansible_host=192.168.1.2 ansible_user=sshuser ansible_password=P@ssword ansible_port=2026
HQ-CLI ansible_host=192.168.2.2 ansible_user=user ansible_password=P@ssword
HQ-RTR ansible_host=192.168.1.1 ansible_user=net_admin ansible_password=P@ssword
BR-RTR ansible_host=192.168.4.1 ansible_user=net_admin ansible_password=P@ssword
EOF
...
# ============================================
# 5. Применение правил sudo для группы sidehq на всех клиентах через Ansible
# ============================================
echo "=== Настройка sudo для группы sidehq на клиентах (HQ-SRV, HQ-CLI, HQ-RTR, BR-RTR) ==="
ansible all -i /etc/ansible/hosts -m copy \
    -a "content='%sidehq ALL=(ALL) NOPASSWD: /bin/cat, /bin/grep, /usr/bin/id\n' dest=/etc/sudoers.d/sidehq mode=0440" \
    || echo "Предупреждение: не удалось настроить sudo на некоторых хостах"
