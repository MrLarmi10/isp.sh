...
# ============================================
# 4. Настройка Ansible (отключаем проверку host key)
# ============================================
echo "=== Настройка Ansible ==="

# Создаём конфиг для ansible, отключающий проверку host key
mkdir -p /etc/ansible
...
# Проверяем, что ansible и sshpass установлены
if command -v ansible &>/dev/null && command -v sshpass &>/dev/null; then
    ...
    echo "Предупреждение: ansible или sshpass не установлены. Пропускаем ping."
fi
