...
# Принудительная очистка суперблоков и создание RAID1
mdadm --zero-superblock --force /dev/sdb /dev/sdc || true
mdadm --create /dev/md0 --level=1 --raid-devices=2 --force --run /dev/sdb /dev/sdc
...
mkdir -p /mnt/iso
mount -o loop /path/to/Additional.iso /mnt/iso    # замените на реальный путь
