#!/bin/bash
# Файл: hq-srv_setup.sh
# Настройка сервера HQ-SRV: RAID1, NFS, Apache + MariaDB, веб-приложение

set -e

# IP-адрес (пример, шлюз 192.168.1.1)
nmcli con mod eth0 ipv4.addresses 192.168.1.2/27 ipv4.method manual
...
# Импорт базы данных (предполагается, что файл dump.sql находится в /root/web)
...
# Копирование файлов приложения в каталог Apache
...
# Разрешение SELinux для NFS и Apache (если включен)
